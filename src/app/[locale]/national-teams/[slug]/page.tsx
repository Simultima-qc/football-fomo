...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `${BASE_URL}/${locale}/national-teams/${slug}`,
      languages: {
        en: `${BASE_URL}/en/national-teams/${slug}`,
        fr: `${BASE_URL}/fr/national-teams/${slug}`,
        "x-default": `${BASE_URL}/en/national-teams/${slug}`,
      },
    },
  };
}
...
export async function generateStaticParams() {
  const entities = await getEntitiesByType("NATIONAL_TEAM");
...
export default async function NationalTeamPage({ params }: Props) {
  ...
  const entity = await getEntityBySlug(slug);
  if (!entity || entity.entityType !== "NATIONAL_TEAM") notFound();
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: locale === "fr" ? "Équipes nationales" : "National Teams", item: `${BASE_URL}/${locale}/national-teams` },
      { "@type": "ListItem", position: 3, name, item: `${BASE_URL}/${locale}/national-teams/${slug}` },
    ],
  };
        ...
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-10">
          ...
          {items.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center">
              <Flag className="w-10 h-10 text-zinc-700 mb-4" />
              <p className="text-zinc-500">
                {locale === "fr"
                  ? "Aucune actualité pour cette équipe pour le moment."
                  : "No news for this national team yet."}
              </p>
            </div>
          )}
