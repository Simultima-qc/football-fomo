...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: locale === "fr" ? "Équipes nationales" : "National Teams",
    description:
      locale === "fr"
        ? "Suivez l'actualité des équipes nationales sur FootballFOMO."
        : "Follow the latest news about national teams on FootballFOMO.",
    alternates: {
      canonical: `${BASE_URL}/${locale}/national-teams`,
      languages: {
        en: `${BASE_URL}/en/national-teams`,
        fr: `${BASE_URL}/fr/national-teams`,
        "x-default": `${BASE_URL}/en/national-teams`,
      },
    },
  };
}

export default async function NationalTeamsIndexPage({ params }: Props) {
  const { locale } = await params;
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {entities.map((entity: any) => {
              ...
              return (
                <Link
                  key={entity.id}
                  href={`/${locale}/national-teams/${entity.slug}`}
